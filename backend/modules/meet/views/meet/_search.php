...
<div class="default-search">
    ...
    <script type="text/javascript">
        cal_s = Calendar.setup({
            weekNumbers: true,
            inputField : "meetsearch-time_s",
            trigger    : "meetsearch-time_s",
            dateFormat: "%Y-%m-%d",
            showTime: true,
            minuteStep: 1,
    ...
    <script type="text/javascript">
        cal_e = Calendar.setup({
            weekNumbers: true,
            inputField : "meetsearch-time_e",
            trigger    : "meetsearch-time_e",
            dateFormat: "%Y-%m-%d",
            showTime: true,
            minuteStep: 1,
            onSelect   : function() {this.hide();}
    ...
    <?= Html::a('报销申请',Yii::$app->urlManager->createUrl(['meet/meet/create']), ['class' => 'btn']) ?>
    <?php } ?>
...
<script>
    $(function(){
        $("#chaxun").click(function(){
            var release = $("#meetsearch-time_s").val();
            var modify = $("#meetsearch-time_e").val();
            if(release>modify){
                if(modify!=''){
                    alert('时间格式错误');
                    $("#meetsearch-time_e").val("");
                    return false;
                }
            }
        });
