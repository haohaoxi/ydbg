...
use backend\modules\kaoqinquery\models\KaoqindaySearch;
...
/**
 * KaoqindayController implements the CRUD actions for KaoqinDay model.
 */
class KaoqindayController extends CommonController
{
...
//                    'delete' => ['post'],
    ...
    /**
     * Lists all KaoqinDay models.
     * @return mixed
     */
    public function actionIndex()
    {
        $searchModel = new KaoqindaySearch();
    ...
    /**
     * Displays a single KaoqinDay model.
     * @param integer $id
     * @return mixed
     */
...
//                $objPHPExcel=$objReader->load($excel_file);
                ...
                if(!empty($execlDataTitle)){
                    if(($execlDataTitle[0]!='部门' || $execlDataTitle[1]!='工号' || $execlDataTitle[4]!='星期')){
                ...
                for($row=2;$row<=$highestRow;$row++){
                    for($col=1;$col<=$highestColumnIndex;++$col){
                        if($col==4){//日期格式处理
                            ...
                            $times=$objWorksheet->getCellByColumnAndRow($col,$row)->getValue();
                            if(strstr($times,'.')){
                                $excelData[$row][$col-1]=gmdate('H:i',PHPExcel_Shared_Date::ExcelToPHP($times));
                            }else{
                                $excelData[$row][$col-1] = $times;
                            }
                        }else{
                ...
                functions::alert('导入成功！',Yii::$app->urlManager->createUrl(['/kaoqinquery/kaoqinday/index']));
            }else{
    ...
    /**
     * Updates an existing KaoqinDay model.
    ...
    /**
     * Deletes an existing KaoqinDay model.
    ...
    /**
     * Finds the KaoqinDay model based on its primary key value.
     * If the model is not found, a 404 HTTP exception will be thrown.
     * @param integer $id
     * @return KaoqinDay the loaded model
     * @throws NotFoundHttpException if the model cannot be found
     */
    protected function findModel($id)
    {
        if (($model = KaoqinDay::findOne($id)) !== null) {
            return $model;
        } else {
