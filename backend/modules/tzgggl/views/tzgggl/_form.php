...
<div style="position: relative; zoom: 1;" id="boxer">
    <?php $form = ActiveForm::begin(['options'=>['enctype'=>'multipart/form-data']]); ?>
    <div class="news-show">
        <?php if (empty($_GET['look'])){?>
            <div class="news-show-top">【新增通知公告】</div>
        <?php }elseif(empty($_GET['look_type'])){?>
            <div class="news-show-top">【修改通知公告】</div>
        <?php }else{?>
            <div class="news-show-top">【通知公告管理】</div>
        <?php }?>
        <div class="news-show-l"><em>*</em>通告标题</div><div class="news-show-r"><?= $form->field($model, 'title')->textInput(['maxlength' => true ,'class' => 'title'])->label('') ?></div>
        <div class="clr"></div>
        <div class="news-show-l">附件</div>
        <div class="news-show-r">
            <?php if(empty($_GET['look_type'])){?>
            <?php echo $form->field($model, 'attachment')->fileInput(['class'=>'upload', 'style'=>'float:left;margin-top:15px;margin-left:10px;','onchange'=>'onUploadFileChange(this);','contentEditable'=>'false'])->label('') ?>
            <em style="float: left;line-height: 50px">(DOC,PDF,DOCX,TXT格式)</em>
            <input type="file" class="hid" id="upload" />
                ...
                <?php endif;?>
            <?php }else{?>
                <?= Html::a($model->attachment,Yii::$app->urlManager->createUrl(['/tzgggl/tzgggl/down','id'=>$model->id]),['style'=>'color:blue;float:left;margin-top:15px;margin-left:5px']) ?>
            <?php }?>
        </div>
        <div class="clr"></div>
        <div class="news-show-l news-show-tall" id="text">正文</div>
        <div class="news-show-r news-show-r-tall" id="text">
    ...
    <div class="news-show-btn">
        ...
        <?= Html::input('submit','dosubmit',Yii::t('app','保存'),['style' => 'margin-left:30px']) ?>
    </div>
        <?php }else{?>
            <div class="news-show-btn">
                <?= Html::a('返回',['index'])?>
