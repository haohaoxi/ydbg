...
/** 获取 artDialog 可跨级调用的最高层�?window 对象 */
var _parent = artDialog.parent = function () {
    var parent = window,
    test = function (name) {
        try {
            var doc = window[name].document;    // 跨域|无权�?
 ...
 * @param   {String}    存储的数据名
 * @param   {Any}       将要存储的任意数�?无此项则返回被查询的数据)
 */
 ...
 * 弹窗 (iframe)
 * @param   {String}    地址
 * @param   {Object}    配置参数. 这里传入的回调函数接收的�?个参数为iframe内部window对象
...
artDialog.open = function (url, options, cache) {
    ...
    var load = function () {
        ...
        if (!loadInit) {
            setTimeout(function () {
                iframe.style.cssText = iframeCss;
            }, 0);// setTimeout: 防止IE6~7导致对话框样式渲染异�?
        };
    ...
    var config = {
        ...
        closeFn: function () {
            ...
            // 重要！需要重置iframe地址，否则下次出现的对话框在IE6�?无法聚焦input
            // IE删除iframe后，iframe仍然会留在内存中出现上述问题，置换src是最容易解决的方�?
